...
#BASE_URL =  "http://10.104.41.159:443/api"
...
def test_tg_auth():
    ...
    with httpx.Client(verify=False) as session:
        ...
        send_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/telegram/send-code"))
        ...
        verify_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/telegram/verify"), json={"magic_token": send_resp.json()['data']['magic_token'], "code": code})
        ...
        me_resp = session.get(urljoin(BASE_URL, "/api/v1/user/me"), headers={"Authorization": f"Bearer {access_token}"})
        ...
        refresh_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/refresh"), cookies={"refresh_token": cookies.get("refresh_token")})
        ...
        set_name_resp = session.patch(urljoin(BASE_URL, "/api/v1/user/display-name"), headers={"Authorization": f"Bearer {access_token}"}, json={"display_name": "MyName2"})
        if set_name_resp.status_code == 200:
            ...
            return
        set_lang_resp = session.patch(urljoin(BASE_URL, "/api/v1/user/language"), headers={"Authorization": f"Bearer {access_token}"}, json={"language": "En"})
        ...
        logout_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/logout"), cookies={"refresh_token": cookies.get("refresh_token")})
...
def test_logout_all():
    ...
    with httpx.Client() as session:
        ...
        send_resp2 = session.post(urljoin(BASE_URL, "/api/v1/auth/telegram/send-code"))
        ...
        verify2_resp = session.post( urljoin(BASE_URL, "/api/v1/auth/telegram/verify"), json={"magic_token": send_resp2.json()['data']['magic_token'], "code": code2})
        ...
        logout_all_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/logout-all"), headers={"Authorization": f"Bearer {access_token2}"}, cookies={"refresh_token": cookies.get("refresh_token")})
...
def test_mail_auth():
    ...
    with httpx.Client() as session:
        ...
        send_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/email/send-code"), json={"email": "lukoninmaksim6@gmail.com"})
        if send_resp.status_code != 200:
            print_error("Не удалось отправить код", send_resp)
            return
        ...
        verify_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/email/verify"), json={"magic_token": send_resp.json()['data']['magic_token'], "code": code})
        if verify_resp.status_code != 200:
            print_error("Ошибка верификации", verify_resp)
            return
        time.sleep(1)
        ...
        print("\n5. Изменение данных пользователя")
        set_tg_resp = session.post(urljoin(BASE_URL, "/api/v1/user/telegram"), headers={"Authorization": f"Bearer {access_token}"})
        ...
        set_tg_resp2 = session.post(urljoin(BASE_URL, "/api/v1/user/telegram/verify"), headers={"Authorization": f"Bearer {access_token}"}, json={"magic_token": data["magic_token"], "code": code})
...
def test_set_avatar():
    print("=== Тестирование установки изображения ===\n")
    with httpx.Client() as session:
        print("1. Отправка кода через Email...")
        send_resp = session.post(urljoin(BASE_URL, "/api/v1/auth/email/send-code"), json={"email": "lukoninmaksim6@gmail.com"})
        if send_resp.status_code != 200:
        ...
        buf.seek(0)
        set_avatar_resp = session.patch(
            urljoin(BASE_URL, "/api/v1/user/avatar"),
            headers={"Authorization": f"Bearer {access_token}"},
            files={"file": ("test_image.jpg", buf.read(), "image/jpeg")}
        )
        print(set_avatar_resp.text)
        if set_avatar_resp.status_code == 200:
            print(f"{GREEN}/api/user/avatar работает корректно{RESET}")
        else:
            print("/api/user/avatar вернул ошибку", set_avatar_resp.status_code)
            return
        time.sleep(1)

        print("\nПолучение информации о пользователе. Проверка /me")
        me_resp = session.get(urljoin(BASE_URL, "/api/v1/user/me"), headers={"Authorization": f"Bearer {access_token}"})
        if me_resp.status_code == 200:
            print(f"{GREEN}/me работает корректно{RESET}")
            print("   Ответ:", json.dumps(me_resp.json(), indent=2, ensure_ascii=False))
        ...
        time.sleep(1)
        input("Нажмите, чтобы продолжить (изменение аватара, предыдущий автомитически удаляется)")
        img = Image.new('RGB', (800, 600), color='green')
        buf = BytesIO()
        img.save(buf, format='JPEG', quality=85)
        buf.seek(0)
        set_avatar_resp2 = session.patch(
            urljoin(BASE_URL, "/api/v1/user/avatar"),
            headers={"Authorization": f"Bearer {access_token}"},
            files={"file": ("test_image.jpg", buf.read(), "image/jpeg")}
        )
        print(set_avatar_resp2.text)
        if set_avatar_resp2.status_code == 200:
            print(f"{GREEN}/api/user/avatar работает корректно{RESET}")
        else:
            print("/api/user/avatar вернул ошибку", set_avatar_resp2.status_code)
            return
